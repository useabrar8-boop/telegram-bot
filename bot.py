...
def get_invidious_url(video_id, quality):
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=10) as response:
                ...
                if quality == "audio":
                    ...
                    target = height_map.get(quality, 720)
                    best = None
                    for f in formats:
                        if f.get('type', '').startswith('video/mp4'):
                            h = f.get('resolution', '0p')
                            h = int(h.replace('p', '')) if h and h != 'None' else 0
                            if h <= target:
                                if best is None or h > int(best.get('resolution', '0p').replace('p', '') or 0):
                                    best = f
                    if best:
                        return best.get('url'), data.get('title', 'video')
        except:
            continue
    return None, None
...
def download_video(url, quality):
    ...
    # YouTube হলে প্রথমে Invidious try করো
    if video_id:
        try:
            direct_url, title = get_invidious_url(video_id, quality)
            if direct_url:
                import urllib.request
    ...
    # Invidious কাজ না করলে yt-dlp দিয়ে চেষ্টা করো
        ...
        try:
            ...
            if quality == "audio":
                await query.message.reply_audio(
                    ...
                    caption="✅ আপনার অডিও রেডি! 🎵"
                )
            else:
                await query.message.reply_video(
                    video=open(file_path, 'rb'),
                    caption=f"✅ {q_name} ভিডিও রেডি! 🎬"
                )
